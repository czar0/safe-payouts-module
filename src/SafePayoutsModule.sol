...
/// @dev As modules could represent a security threat to your Safe account, the following implementation measures are recommended for production-ready usage:
...
contract SafePayoutsModule {
    ...
    function removePayout(address beneficiary) public onlyOperator(msg.sender) {
        ...
        beneficiaries.pop();
        delete paid[beneficiary];
        delete payouts[beneficiary];
    }
    ...
    function executePayouts(address safe) public onlyOperator(msg.sender) {
    ...
    function _transfer(address safe, address token, address payable to, uint256 amount) private {
        if (token == address(0)) {
            require(
                ISafeModuleManager(safe).execTransactionFromModule(to, amount, "", ISafeModuleManager.Operation.Call),
                "could not execute ether transfer"
            );
        } else {
            bytes memory data = abi.encodeCall(ISafeModuleManager.transfer, (to, amount));
            require(
                ISafeModuleManager(safe).execTransactionFromModule(token, 0, data, ISafeModuleManager.Operation.Call),
...
/// @dev Using interfaces reduces the amount of code imported by the main contract and, consequently, the gas fees during the deployment phase
/// However, interfaces can expose to incompatibility issues or errors in case the original contracts they refer to changed
/// Before deploying the contract, it is recommended to verify the correctness of all methods and parameters or to refactor the code to use imports rather than interfaces
